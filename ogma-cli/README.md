...
## Standalone Monitor Generation
<sup>[(Back to top)](#table-of-contents)</sup>

Ogma can turn a specification with requirements or properties into a monitor
implemented in Copilot, without targeting a specific platform like cFS, ROS 2
or F'. This is useful to inspect the Copilot code generated from a
specification, and to integrate monitors into systems not directly supported
by other Ogma backends.

For example, the file `ogma-cli/examples/csv/requirements.csv`, included with
the Ogma distribution, contains several requirements over two external
variables `autopilot` and `gps_fix`, with their formalizations written in SMV
in the fourth column. The first requirement in that file is:
...
The following command generates a Copilot monitor that implements those
requirements:

```sh
$ ogma standalone --input-file ogma-cli/examples/csv/requirements.csv --input-format ogma-cli/examples/csv/csv-format.cfg
```

By default, the output is placed in a directory `copilot`, which contains a
Copilot module `Copilot.hs`. Compiling that module with Copilot installed
produces the C99 implementation of the monitors.

The `standalone` command supports the following arguments:

- `--target-dir DIR`: Use this argument to provide a target directory for the
  files generated (default: "copilot").
- `--template-dir DIR`: Use this argument to provide a custom template for the
  code being generated. See the files under `ogma-core/templates/standalone/`,
as well as other commands supporting template customization, to understand how
to customize the template.
- `--condition-expr EXPRESSION`: Condition upon which the monitor will fire or
  notify, provided directly as an expression in the property language
selected. This argument can be used instead of an input file to generate a
monitor from a single property given at the command line.
- `--input-file FILENAME`: File with the properties or requirements being
  transformed. This argument can be provided multiple times to read several
input files of the same format.
- `-f,--input-format FORMAT_NAME`: Name of the input format, or path to a
  custom format specification file. See below for details.
- `-p,--prop-format FORMAT_NAME`: Format for the temporal or boolean
  properties in the specification. See below for details.
- `-m,--map-type TYPE_NAME:TYPE_NAME`: Map a type name used in the
  specification to a different type in the generated code. This argument can
be provided multiple times.
- `--target-file-name FILENAME`: Filename prefix for monitoring files in
  target language (default: "monitor").
...
- `--template-vars FILENAME`: JSON file containing additional variables to
  expand in the template.

### Input Specification Formats

The argument `--input-format` accepts either the name of a format included
with Ogma, or a path to a file with a custom format specification.

The formats included with Ogma are `fcs`, for files generated by NASA's
[FRET](https://github.com/NASA-SW-VnV/fret), and `fdb`, for FRET requirement
databases. Custom format specification files describe how to locate the
variables and the requirements in the input file, and support input files in
JSON, YAML, CSV, XLSX and XML. For examples of custom format specifications,
see the files `json-format.cfg`, `csv-format.cfg` and `xlsx-format.cfg` under
`ogma-cli/examples/`.

For example, the following command generates a monitor from a requirements
specification in XLSX format, also included with the Ogma distribution:

```sh
$ ogma standalone --input-file ogma-cli/examples/xlsx/requirements.xlsx --input-format ogma-cli/examples/xlsx/xlsx-format.cfg
```

### Property Specification Formats

Ogma supports properties specified in multiple languages, such as:

- `smv`: The properties are boolean or temporal SMV expressions (default).

- `lustre`: The properties are boolean or temporal Lustre expressions. The
  format name `cocospec` is accepted as a synonym for `lustre`.

- `literal`: The properties should be passed literally to the target file,
  that is, they should be boolean Copilot streams.

Use the argument `--prop-format` to customize the name of the format being
used. Properties in other languages can be supported by pre-processing them
with an external command, provided via the argument `--parse-prop-via`.
