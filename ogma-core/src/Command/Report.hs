...
-- AGREEMENT.
--
-- | Produce an report of the input files.
...
-- | Generate report of spec given in an input file.
--
...
-- the variables needed by the report application generator. The target
...
commandResult _options fp result = case result of
  Left msg -> (Nothing, Error ecReportError msg (LocationFile fp))
